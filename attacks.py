...
def make_diff_jpeg(images):
    ...
    images_jpeg_25 = jpeg_diff_25_fn(images.cpu()).cuda()
    images_jpeg_50 = jpeg_diff_50_fn(images.cpu()).cuda()
    images_jpeg_75 = jpeg_diff_75_fn(images.cpu()).cuda()
...
def fgsm_jpeg(model, criterion, images, labels, target=None):
    eps = 1
    alpha = 3
    num_iter = 10
...
def fgsm(model, criterion, images, labels, target=None):
    eps = 3
    alpha = 3
    num_iter = 10
    inp = Variable(images, requires_grad=True)
